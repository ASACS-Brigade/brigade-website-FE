...
const loopedTestimonials = Array.from({ length: 3 }, (_, copyIndex) =>
  testimonials.map((testimonial, logicalIndex) => ({
    testimonial,
    logicalIndex,
...
export default function Testimonials() {
  const [virtualIndex, setVirtualIndex] = useState<number>(TESTIMONIAL_COUNT + 1);
  const [isMobile, setIsMobile] = useState<boolean>(false);
  const [transitionEnabled, setTransitionEnabled] = useState<boolean>(true);
  const autoScrollInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  const resumeTimeout = useRef<ReturnType<typeof setTimeout> | null>(null);
  const activeIndex = ((virtualIndex % TESTIMONIAL_COUNT) + TESTIMONIAL_COUNT) % TESTIMONIAL_COUNT;
  ...
  */
  const getTranslateX = useCallback(() => {
    const cardCentre = virtualIndex * (cardWidth + cardGap) + cardWidth / 2;
    return -cardCentre;
  }, [cardGap, cardWidth, virtualIndex]);

  const startAutoScroll = useCallback(() => {
    if (autoScrollInterval.current) clearInterval(autoScrollInterval.current);
    autoScrollInterval.current = setInterval(() => {
      setVirtualIndex((previous) => previous + 1);
    }, 3000);
  }, []);
  ...
  const goTo = (index: number) => {
    ...
    const candidates = [index, index + TESTIMONIAL_COUNT, index + TESTIMONIAL_COUNT * 2];
    const nearestIndex = candidates.reduce((nearest, candidate) =>
      Math.abs(candidate - virtualIndex) < Math.abs(nearest - virtualIndex) ? candidate : nearest,
    );
  ...
  const handleLoopTransitionEnd = () => {
    if (virtualIndex >= TESTIMONIAL_COUNT * 2) {
      setTransitionEnabled(false);
      setVirtualIndex((previous) => previous - TESTIMONIAL_COUNT);
      requestAnimationFrame(() => requestAnimationFrame(() => setTransitionEnabled(true)));
    } else if (virtualIndex < TESTIMONIAL_COUNT) {
      setTransitionEnabled(false);
      setVirtualIndex((previous) => previous + TESTIMONIAL_COUNT);
      requestAnimationFrame(() => requestAnimationFrame(() => setTransitionEnabled(true)));
    }
    ...
    */
    <section className="flex flex-col items-center py-8 md:py-16 gap-8 w-full max-w-360 mx-auto min-h-125 md:min-h-157.5 overflow-x-hidden">
      ...
      <div className="text-center mb-4 px-4 w-full">
        ...
        {/*
          Avatar strip:
          - Avatars stay on one centred row at every breakpoint.
          - Phone-sized avatars and gaps keep the full row inside narrow screens.
        */}
        <div className="flex flex-nowrap justify-center gap-4 mt-10 px-4 py-2 max-[582px]:gap-1.5 max-[582px]:px-2">
          {testimonials.map((testimonial, idx) => (
            ...
            >
              <Image
                ...
                className="object-cover rounded-full w-12 h-12 md:w-16 md:h-16 max-[582px]:h-8 max-[582px]:w-8 border-2 transition-all"
                style={{
      ...
      <div className="w-full flex flex-col items-center gap-6">
        ...
        */}
        <div className="relative h-80 w-full overflow-x-hidden md:h-96">
          {/*
            Track — absolutely positioned, full natural width, no overflow.
            Its left edge is anchored at the viewport centre and translateX(...)
            aligns the selected card over that anchor. Three card sets provide
            enough runway for an invisible loop reset after either boundary.
          */}
          <div
            className="absolute top-0 left-1/2 flex items-center h-full gap-3 md:gap-6"
            style={{
            ...
            onTransitionEnd={(event) => {
              if (event.target === event.currentTarget) handleLoopTransitionEnd();
            }}
          >
            {loopedTestimonials.map(({ testimonial, logicalIndex, key }, trackIndex) => {
              const isActive = virtualIndex === trackIndex;
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${
                      isActive
                        ? "p-4 md:p-[26.46px] scale-100 shadow-lg z-10"
                        : "p-3 md:p-[20.89px] scale-90 md:scale-95 opacity-70 md:opacity-85 z-0 mt-4 md:mt-6"
                    }
                  `}
                  style={{
                    width: `${cardWidth}px`,
